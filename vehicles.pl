...
my $sth = $dbh->prepare("delete from objects using objects inner join survivals on objects.oid = survivals.id and survivals.is_dead = 1 where objects.otype = 'TentStorage' and objects.lastupdate < now() - interval 4 day") or die;
