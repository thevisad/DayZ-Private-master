...
given (scalar(@ARGV))
{
	when(5) {$dbinstance = $ARGV[0];$db = $ARGV[1];$dbuser= $ARGV[2];$pw = $ARGV[3];$globalLimit=$ARGV[4];}
	...
	default {die "You can provide up to 5 arguments each separated by spaces in the following order:\n\t2 args: instance passToDB\n\t3 args: instance userDB passToDB\n\t4 args: instance database userDB passToDB\n;\t5 args: instance database userDB passToDB maxNumberOfVehicles\n"
			}
}
...
{
	print "Reached maximum vehicle limit for these types of vehicles! Not spawning any vehicles...";
}
sub genDamage
{
