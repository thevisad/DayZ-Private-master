...
diag_log("MOTHER");
	...
	// we get the player uid so we retrieve id position backpack inventory medical
		_muid = _result select 2;
			...
			_ret = ["",1,call compile (_qresult select 0),call compile (_qresult select 1),call compile (_qresult select 2),call compile ([_qresult select 3,"["",","["""","] call CBA_fnc_replace),call compile (_qresult select 4),[call compile (_qresult select 5),call compile (_qresult select 8),call compile (_qresult select 9)],[call compile (_qresult select 6),call compile (_qresult select 10),call compile (_qresult select 11),call compile (_qresult select 12)],call compile ([_qresult select 7,"[""|","[""""|"] call CBA_fnc_replace)];
	}
	else
	{
		...
		if(_qresult!="[[]]") then
		{
			...
			_qresult = call compile _qresult;
			_qresult = _qresult select 0;
			_ret = ["",1,call compile (_qresult select 0),0,call compile (_qresult select 1),call compile ([_qresult select 2,"["",","["""","] call CBA_fnc_replace),[call compile (_qresult select 3),call compile (_qresult select 5),call compile (_qresult select 6)],call compile (_qresult select 4),dayz_hiveVersionNo];
		} else
		{
