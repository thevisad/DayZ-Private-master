...
//Get message page count
_result = "blisshive" callExtension format ["Q:%1:call proc_getMessagePageCount(%2)", (call fnc_instanceName), dayz_instance];
_result = call compile _result;
_pageCount = call compile ((_result select 0) select 0);
diag_log("SERVER: Got " + str(_pageCount max 1) + " pages of messages");

//Load messages
msgList = [];
_msgCount = 0;
for "_page" from 1 to _pageCount do {
	_result = "blisshive" callExtension format ["Q:%1:call proc_getMessagePage(%2, %3)", (call fnc_instanceName), dayz_instance, _page];
	_result = call compile _result;
	_end = ((count _result) - 1);
	for "_i" from 0 to _end do {
		_item = _result select _i;
		if (count _item > 0) then {
			_item set [1, (call compile (_item select 1))];
			_item set [2, (call compile (_item select 2))];
			msgList set [count msgList, _item];
			_msgCount = _msgCount + 1;
			//diag_log("DEBUG: Added message " + _item);
		};
	};
};
diag_log("SERVER: Added " + str(_taskCount) + " messages!");
