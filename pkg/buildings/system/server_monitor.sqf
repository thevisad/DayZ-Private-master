...
//Get building page count
_result = "blisshive" callExtension format ["Q:%1:call proc_getBuildingPageCount(%2)", (call fnc_instanceName), dayz_instance];
_result = call compile _result;
_pageCount = call compile ((_result select 0) select 0);
diag_log("SERVER: Got " + str(_pageCount max 1) + " pages of buildings...");

//Load buildings
_bldCount = 0;
for "_page" from 1 to _pageCount do {
	_result = "blisshive" callExtension format ["Q:%1:call proc_getBuildings(%2, %3)", (call fnc_instanceName), dayz_instance, _page];
	_result = call compile _result;
	_end = ((count _result) - 1);
	for "_i" from 0 to _end do {
		_item = _result select _i;
		if (count _item > 0) then {
			_building = createVehicle [_item select 0, _item select 1, [], 0, "CAN_COLLIDE"];
